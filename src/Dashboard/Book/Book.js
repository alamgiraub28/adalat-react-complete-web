...
const Book = () => {
    ...
    const { title, price } = serviceDetail;
    useEffect(() => {
        fetch(`https://agile-springs-09657.herokuapp.com/service/${id}`)
            .then(res => res.json())
    ...
    const  handlePaymentSuccess= (paymentId) => {
        ...
        fetch('https://agile-springs-09657.herokuapp.com/addOrder', {
            method: 'POST',
            headers: {
                ...
                <Row>
                    <Col lg={6} md={6} sm={12} className="p-5 m-auto rounded card" style={{display: bookingData ? 'none': 'block'}}>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <input className="form-control" value={title} {...register("name")} /> <br/>
                            ...
                            <input className="btn btn-custom btn-block" type="submit" />
                        </form>
                    </Col>
